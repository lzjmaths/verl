...
if [ "$TASK" = "AIME" ]; then
  ...
  echo "警告: TASK 不是 AIME24/25 或 MATH500，使用默认值。"
  TEST_TASK="$TASK"
fi
BACKBONE="Qwen2.5-Math-1.5B"
ADVANTAGE="grpo"

DATA_LOCAL_DIR="/data/private/linzejin/proofrl/data"
BACKBONE_PATH="/data/private/linzejin/models/Qwen/${BACKBONE}"
...
uv run -i https://pypi.tuna.tsinghua.edu.cn/simple --active python -m verl.trainer.main_ppo \
    ...
    custom_reward_function.path="/data/private/linzejin/proofrl/verl/RLVR/math_reward.py"  $@ 
