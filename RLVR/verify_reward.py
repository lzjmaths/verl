...
client = openai.OpenAI(
    api_key="not-needed-for-vllm",  # API Key 不是必需的，但字段需要存在
    base_url="http://localhost:10000/v1"  # <-- 这里填写你的基础 URL
)
...
def compute_score(data_source:str, solution_str:str, ground_truth, extra_info=None, timeout_score=0.0) -> bool:
    ...
    reviewer = "/data3/private/linzejin/models/Qwen3-14B"  # 默认值
    reviews = 4
    workers = 4
    # try:
...
# print(compute_score("lighteval/MATH","prove 1+1=2", "because 1+1=2", "true"))
